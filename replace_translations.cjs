...
try {
  const pagePath = path.join(__dirname, 'src/pages/DonationTrackerPage.tsx');
  ...
  for (const [koString, mapping] of Object.entries(translations)) {
    enJson[mapping.key] = mapping.en;
    koJson[mapping.key] = koString;

    // Replace t('...') or t("...") in content
    // Need to escape regex characters in koString if any
    const escaped = koString.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const regex = new RegExp(`t\\(['"]${escaped}['"]\\)`, 'g');
    content = content.replace(regex, `t('tracker.${mapping.key}')`);
  }
  ...
  // Merge with existing locale files
  const existingEnJson = JSON.parse(fs.readFileSync(enPath, 'utf8'));
  const existingKoJson = JSON.parse(fs.readFileSync(koPath, 'utf8'));

  existingEnJson.tracker = { ...existingEnJson.tracker, ...enJson };
  existingKoJson.tracker = { ...existingKoJson.tracker, ...koJson };

  fs.writeFileSync(enPath, JSON.stringify(existingEnJson, null, 2));
  fs.writeFileSync(koPath, JSON.stringify(existingKoJson, null, 2));

  console.log("Updated locale files!");
  console.log("== EN JSON ==");
  console.log(JSON.stringify({ tracker: enJson }, null, 2));
  console.log("== KO JSON ==");
  console.log(JSON.stringify({ tracker: koJson }, null, 2));
} catch (error) {
  console.error("Failed to replace translations:", error.message);
  process.exit(1);
}
